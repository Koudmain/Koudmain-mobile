...
export const useCalendarEvents = (
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      if (missingMonths.length === 0) {
        ...
        setEvents((prevEvents) =>
          prevEvents.filter((evt) => {
            ...
            return (
              new Date(evt.starting_date).getTime() >= minAllowedTime &&
              new Date(evt.starting_date).getTime() <= maxAllowedTime
            );
          }),
        );
        return;
      ...
      try {
        if (!activeCompanyId) {
          console.warn('Missing activeCompanyId');
          return;
        }
        const response =
          loadedMonths.current.size === 0
            ? await planningService.getPlanning(session, activeCompanyId)
            : await planningService.getPlanning(
                session,
                activeCompanyId,
                fetchStartDate,
                fetchEndDate,
    ...
    fetchEvents();
  }, [currentMonthStr, session, activeCompanyId]);
